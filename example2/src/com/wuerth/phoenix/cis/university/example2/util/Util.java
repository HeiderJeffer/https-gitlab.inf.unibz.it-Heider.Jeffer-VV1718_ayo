...
public class Util {
	...
	public static String getName(ConcreteAccount concreteAccount, Locale locale) {
		if(concreteAccount != null) {
	        ...
	        case Constants.IFRS16_RIGHT_OF_USE_LAND_OFFICES:
	        	return tr("Nutzungsrecht Grundstücke mit Geschäfts-, Fabrik- und anderen Bauten", "Right of use land together with office, factory and other buildings", locale);
	        case Constants.IFRS16_RIGHT_OF_USE_LAND_RESIDENTIALS:
	        	return tr("Nutzungsrecht Grundstücke mit Wohnbauten", "Right of use land together with residential buildings", locale);
	        case Constants.IFRS16_RIGHT_OF_USE_LAND_NO_BUILDINGS:
	        	return tr("Nutzungsrecht Grundstücke ohne Bauten", "Right of use land without buildings", locale);
	        case Constants.IFRS16_RIGHT_OF_USE_BUILDINGS:
	        	return tr("Nutzungsrecht Bauten auf fremden Grundstücken", "Right of use buildings constructed on land owned by non-Group", locale);
	...
	public static String getName(IFRS16ConditionType ifrs16ConditionType, Locale locale) {
		if(ifrs16ConditionType != null) {
	        ...
	        case Constants.CONDITIONTYPE_CODE_RentI:
	        	return tr("Miete I", "Rent I", locale);
	        ...
	        case Constants.CONDITIONTYPE_CODE_RentII:
	        	return tr("Miete II", "Rent II", locale);
	        case Constants.CONDITIONTYPE_CODE_RentIII:
	        	return tr("Miete III", "Rent III", locale);
	 ...
	 */
	public static boolean isAvailable(IFRS16ImportAssignmentType ifrs16ImportAssignmentType, boolean isLongVersion) {
		if(isLongVersion) {
			return true;
		}
		else {
	 ...
	 */
	public static boolean isValid(boolean isLongVersion, Collection<IFRS16ImportAssignmentType> ifrs16ImportAssignmentTypeList) {
		HashMap<String, HashSet<IFRS16ImportAssignmentType>> errorMap = validateColumnAssociations(isLongVersion, ifrs16ImportAssignmentTypeList);
		int count = 0;
	 ...
	 */
	public static HashMap<String, HashSet<IFRS16ImportAssignmentType>> validateColumnAssociations(boolean isLongVersion, Collection<IFRS16ImportAssignmentType> ifrs16ImportAssignmentTypeList) {
		...
		if(isLongVersion) {
			...
			boolean hasCostCenter = columnTypeSet.contains(IFRS16ImportAssignmentType.COSTCENTER);
